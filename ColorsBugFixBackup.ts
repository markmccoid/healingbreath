...
function getMatchers(): Matchers {
  "worklet";
  const cachedMatchers: Matchers = _WORKLET ? uiCachedMatchers : jsCachedMatchers;
...
export const rgbaColor = (r: number, g: number, b: number, alpha = 1): number | string => {
  "worklet";
...
export const hsvToColor = (h: number, s: number, v: number): number | string => {
  "worklet";
  const { r, g, b } = HSVtoRGB(h, s, v);
  return rgbaColor(r, g, b);
};

export function processColorInitially(color: unknown): number | null | undefined {
  "worklet";
...
const getInterpolateCacheRGBA = (
  ...
  "worklet";
  // -- Bug fix by mrm - https://github.com/software-mansion/react-native-reanimated/issues/2739
...
const getInterpolateCacheHSV = (colors: readonly (string | number)[]): InterpolateCacheHSV => {
  "worklet";
  const hash = colors.join("");
